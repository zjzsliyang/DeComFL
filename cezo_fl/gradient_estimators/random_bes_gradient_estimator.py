...
class BernoulliSmoothGradientEstimator(AbstractGradientEstimator):
    def __init__(
        self,
    ...
    def generate_perturbation_norm(self, rng: torch.Generator | None = None) -> torch.Tensor:
        p = torch.full((self.total_dimensions,), 0.5, device=self.device, dtype=self.torch_dtype)
        sample = torch.bernoulli(p, generator=rng)
    ...
    def generate_then_put_grad_paramwise(self, seed: int, dir_grads: torch.Tensor) -> None:
        ...
        for i, dir_grad in enumerate(dir_grads):
            ...
            for param in self.parameters_list:
                p = torch.full_like(param, 0.5)
                sample = torch.bernoulli(p, generator=rng)
                _perturb = (sample - 0.5) / 0.5
                if i == 0:
    ...
    def perturb_model_paramwise(self, rng: torch.Generator, alpha: float | int) -> None:
        num_params = len(self.parameters_list)
        num_perturb = num_params // 2 # 50% parameters will be perturbed
        indices = torch.randperm(num_params, generator=rng)[:num_perturb]
        
        for i in indices:
            param = self.parameters_list[i]
            p = torch.full_like(param, 0.5)
            sample = torch.bernoulli(p, generator=rng)
            _perturb = (sample - 0.5) / 0.5
